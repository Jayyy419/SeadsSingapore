...
export function SubmitStoryContent() {
  const { t } = useLocale();
  const [status, setStatus] = useState<"idle" | "loading" | "done" | "error" | "unconfigured" | "rateLimited">("idle");

  async function onSubmit(formData: FormData) {
    const baseUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
    if (!baseUrl) {
      // Don't claim success when nothing was actually captured — matches InterestForm's
      // distinction between "misconfigured endpoint" and a genuine submission error.
    ...
    <SiteShell title={t.submitStoryPageTitle} subtitle={t.submitStoryPageSubtitle}>
      <form action={onSubmit} className="section-card grid gap-3 p-6 sm:p-8">
        ...
        <div aria-live="polite">
          ...
          {status === "unconfigured" && <p className="text-xs font-semibold text-[#e2965f]">{t.statusUnconfigured}</p>}
